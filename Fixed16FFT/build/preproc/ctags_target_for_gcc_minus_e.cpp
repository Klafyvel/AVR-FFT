# 1 "/home/klafyvel/Documents/projets/dev/fftduino/first_instructable/Fixed16FFT/Fixed16FFT.ino"
/*
  Fixed16FFT.ino
...
*/
# 9 "/home/klafyvel/Documents/projets/dev/fftduino/first_instructable/Fixed16FFT/Fixed16FFT.ino" 2
# 10 "/home/klafyvel/Documents/projets/dev/fftduino/first_instructable/Fixed16FFT/Fixed16FFT.ino" 2
...
uint8_t fft(fixed_t x[], const int size) {
  ...
  */
  ...
  for(i=0; i<array_num_bits; ++i){
    ...
    cj = FIXED_ONE;
    sj = FIXED_ZERO;
# 159 "/home/klafyvel/Documents/projets/dev/fftduino/first_instructable/Fixed16FFT/Fixed16FFT.ino"
...
float modulus(fixed_t x[], const int size, float frequency) {
  ...
  fixed_t maxi=0;
  for(i=0; i<size/2; i++) {
    a = ((x[2*i])>0?(x[2*i]):-(x[2*i]));
    b = ((x[2*i+1])>0?(x[2*i+1]):-(x[2*i+1]));
    x[i] = ((fixed_mul(ONE_OVER_SQRT_TWO, fixed_add_saturate(a, b)))>(((a)>(b)?(a):(b)))?(fixed_mul(ONE_OVER_SQRT_TWO, fixed_add_saturate(a, b))):(((a)>(b)?(a):(b))));
