...
// Note : I also modified a little bit the end of the function so that we
// can retrieve the output outside of it.
...
{ 
  ...
  for(int i=0;i<Pow2[o-1];i++)               // getting amplitude from compex number
  { 
    // klafyvel : Here I modified the code to use in instead of out_r so that we can 
    // retrieve the fft outside of the function.
